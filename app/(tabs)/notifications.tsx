...
const GameInviteRow = ({ game, refresh }: GameInviteProps) => {
	...
	const accept = useCallback(async () => {
		...
		router.navigate({ pathname: '/(tabs)/game', params: { id: game.id } });
...
const getItemRender = (
	...
	switch (item.type) {
		case 'invite':
			return (
				<GameInviteRow game={item.data} key={item.data.id} refresh={refresh} />
			);
	}
};
...
export default function Notifications() {
	...
	const refresh = useCallback(async () => {
		await Promise.all([invitesRefetch()]);
	}, [invitesRefetch]);

	const items: NotificationItem[] = useMemo(() => {
		if (!gameInvites) return [];

		const inviteItems: NotificationItem[] = gameInvites.map((x) => ({
			id: x.id,
			type: 'invite',
