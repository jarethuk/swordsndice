...
const UnitRow = ({ member, canUpdate, game, memberId, groupId, setList }: HeroRowProps) => {
  const [open, setOpen] = useState(false);
  const [stats, setStats] = useState<RemainingMESBGProfileStats>(member.fullRemainingStats);
  ...
  const update = useCallback(async () => {
    ...
    const list = game.members.find((member) => member.user?.id === memberId)?.list;
  ...
  return (
    <Pressable
      ...
      className={'border-border-light dark:border-border-dark flex gap-2 rounded-2xl border-2 p-4'}>
      ...
      {open ? (
        <>
          <View className={'flex flex-row items-center justify-evenly gap-2'}>
            <StatsControl title={'M'} value={stats.might} onChange={onChange} stat={'might'} />
            <StatsControl title={'W'} value={stats.will} onChange={onChange} stat={'will'} />
          </View>

          <View className={'flex flex-row items-center justify-evenly gap-2'}>
            <StatsControl title={'F'} value={stats.fate} onChange={onChange} stat={'fate'} />
            <StatsControl title={'Wo'} value={stats.wounds} onChange={onChange} stat={'wounds'} />
          </View>
        </>
      ) : (
        <HeroPoints
    ...
    <View className={'flex gap-6'}>
      {groups.map((group, index) => (
        <View key={index.toString()} className={'flex gap-4'}>
          ...
          <UnitRow member={group.leader} {...props} groupId={group.id} setList={setList} />
