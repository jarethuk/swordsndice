...
describe('CodeEnterInput', () => {
  ...
  it('renders correct number of inputs', () => {
    const { getAllByRole } = render(<CodeEnterInput length={6} onComplete={mockOnComplete} />);
    const inputs = getAllByRole('none');
    expect(inputs).toHaveLength(6);
  });

  it('accepts single digit input', () => {
    const { getAllByRole } = render(<CodeEnterInput length={4} onComplete={mockOnComplete} />);
    const inputs = getAllByRole('none');
